...
export default function Studio() {
    ...
    if (canvasRef.current && !canvas) {
      try {
        ...
        if (!fabricCanvas.getElement || !fabricCanvas.getElement()) {
          console.error('Failed to initialize canvas element');
          return;
        }
  ...
  useEffect(() => {
    if (canvas && canvas.getElement && canvas.getElement()) {
      try {
        ...
        console.warn('Canvas resize warning:', error);
      }
    }
  }, [canvas, canvasSize]);
  ...
  useEffect(() => {
    if (canvas && canvas.getElement && canvas.getElement()) {
      try {
        ...
        console.warn('Canvas zoom warning:', error);
      }
    }
  }, [canvas, zoom]);
  ...
  const handleTemplateSelect = (template: any) => {
    ...
    // Load template into existing Fabric.js canvas
    if (canvas && (template.imageUrl || template.previewUrl)) {
      fabric.Image.fromURL(template.imageUrl || template.previewUrl, (img) => {
  ...
  const renderSidebarContent = () => {
    switch (activeTab) {
      case "templates":
        return <TemplatesPanel onSelectTemplate={handleTemplateSelect} />;
      case "text":
        return <TextPanel />;
      case "shapes":
        return <ShapesPanel />;
          ...
          {!selectedTemplate && (
            <div className="absolute inset-0 flex items-center justify-center text-gray-500 pointer-events-none">
              <div className="text-center">
                ...
                <p className="text-lg font-medium">Pick a template to start editing!</p>
              </div>
            </div>
          )}
        </div>
