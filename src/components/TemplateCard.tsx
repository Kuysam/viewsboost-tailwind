...
function pickVariant(category: string | undefined, tags: string[] | undefined): Variant {
  ...
  if (/instagram|reel|story|carousel/.test(hay))
    return choose({ label: 'Instagram', accentFrom: '#fd1d1d', accentTo: '#833ab4', chipBg: 'bg-pink-600', chipText: 'text-white', platformIcon: '/platform-logos/instagram.svg' });
  if (/youtube|thumb|banner/.test(hay))
    return choose({ label: 'YouTube', accentFrom: '#ff4d4f', accentTo: '#b91c1c', chipBg: 'bg-red-600', chipText: 'text-white', platformIcon: '/platform-logos/youtube.svg' });
  if (/tiktok/.test(hay))
    return choose({ label: 'TikTok', accentFrom: '#00f0ff', accentTo: '#ff0050', chipBg: 'bg-black', chipText: 'text-white', platformIcon: '/platform-logos/tiktok.svg' });
  if (/facebook/.test(hay))
    return choose({ label: 'Facebook', accentFrom: '#1877f2', accentTo: '#0b5cd5', chipBg: 'bg-blue-600', chipText: 'text-white', platformIcon: '/platform-logos/facebook.svg' });
  if (/twitter|x\b/.test(hay))
    return choose({ label: 'X', accentFrom: '#0a0a0a', accentTo: '#2d2d2d', chipBg: 'bg-neutral-900', chipText: 'text-white', platformIcon: '/platform-logos/twitter.svg' });
  if (/linkedin/.test(hay))
    return choose({ label: 'LinkedIn', accentFrom: '#0a66c2', accentTo: '#004182', chipBg: 'bg-sky-700', chipText: 'text-white', platformIcon: '/platform-logos/linkedin.svg' });
  if (/pinterest/.test(hay))
    return choose({ label: 'Pinterest', accentFrom: '#e60023', accentTo: '#b0001a', chipBg: 'bg-red-700', chipText: 'text-white', platformIcon: '/platform-logos/pinterest.svg' });
  if (/twitch/.test(hay))
    return choose({ label: 'Twitch', accentFrom: '#9146ff', accentTo: '#5b2dd8', chipBg: 'bg-violet-700', chipText: 'text-white', platformIcon: '/platform-logos/twitch.svg' });

  // Content types
  if (/short|reel|story|9:16/.test(hay))
    return choose({ label: 'Shorts/Story', accentFrom: '#10b981', accentTo: '#06b6d4', chipBg: 'bg-emerald-600', chipText: 'text-white' });
  if (/thumb/.test(hay))
    return choose({ label: 'Thumbnail', accentFrom: '#ef4444', accentTo: '#f97316', chipBg: 'bg-red-600', chipText: 'text-white' });
  if (/document|doc|report|resume|proposal|invoice/.test(hay))
    return choose({ label: 'Document', accentFrom: '#3b82f6', accentTo: '#6366f1', chipBg: 'bg-blue-600', chipText: 'text-white' });
  if (/poster|flyer|print|brochure|business card/.test(hay))
    return choose({ label: 'Print', accentFrom: '#22c55e', accentTo: '#84cc16', chipBg: 'bg-green-600', chipText: 'text-white' });
  if (/marketing|promo|sale|coupon|product/.test(hay))
    return choose({ label: 'Promo', accentFrom: '#f59e0b', accentTo: '#d946ef', chipBg: 'bg-amber-500', chipText: 'text-black' });
  if (/business|branding|logo/.test(hay))
    return choose({ label: 'Business', accentFrom: '#06b6d4', accentTo: '#3b82f6', chipBg: 'bg-cyan-600', chipText: 'text-white' });
